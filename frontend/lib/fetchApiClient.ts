...
function useAuthApi() {
  ...
  const refreshToken = async (refreshToken: string) => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_MANAGEMENT_API_URL}/auth/refresh`, {
      method: "POST",
  ...
  const authGetApi = async (url: string) => {
    ...
    console.log("process.env.NEXT_PUBLIC_MANAGEMENT_API_URL", process.env.NEXT_PUBLIC_MANAGEMENT_API_URL)
    let res = await fetch(`${process.env.NEXT_PUBLIC_MANAGEMENT_API_URL}${url}`, {
      method: "GET",
      headers: {
    ...
    if (res.status === 401 && session?.user?.refreshToken) {
      try {
        ...
        console.log(process.env.NEXT_PUBLIC_MANAGEMENT_API_URL)
        res = await fetch(`${process.env.NEXT_PUBLIC_MANAGEMENT_API_URL}${url}`, {
          method: "GET",
          headers: {
