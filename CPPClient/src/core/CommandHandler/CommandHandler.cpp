...
#ifdef _WIN32
  #include "Attacks/Collection/Windows/basic_info.hpp"
#else
  #include "Attacks/Collection/Linux/basic_info.hpp"
#endif
...
std::string CommandHandler::onRunCommand(const Document& req)
{
    ...
    std::string cmd;
    json::getString(req, "command", cmd);                       // tolerate both “command”: "dir" or nested
...
std::string CommandHandler::makeSuccess(const Value& data)
{
    ...
    d.AddMember("data",    Value(data, a), a);   // deep copy
