...
WsClient::WsClient(const std::string& uri, Logger& logger)
    : uri_(uri), logger_(logger) {
...
bool WsClient::start() {
    ...
    if (ec) {
        logger_.log(std::string("Connection init failed: ") + ec.message(), Logger::Level::Error);
        return false;
    }
    hdl_ = con->get_handle();
...
void WsClient::stop() {
    if(running_) {
        ...
        if(ec) {
            logger_.log(std::string("Close error: ") + ec.message(), Logger::Level::Error);
        }
        if(thread_.joinable()) {
            thread_.join();
        }
...
void WsClient::send(const std::string& msg) {
    ...
    if(ec) {
        logger_.log(std::string("Send failed: ") + ec.message(), Logger::Level::Error);
    }
}

void WsClient::on_open(websocketpp::connection_hdl hdl) {
    logger_.log("WebSocket connection opened", Logger::Level::Info);
}

void WsClient::on_message(websocketpp::connection_hdl, client::message_ptr msg) {
    logger_.log(std::string("Received: ") + msg->get_payload(), Logger::Level::Debug);
}
